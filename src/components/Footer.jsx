    ...
    <footer style={{ background: '#131210', borderTop: '1px solid rgba(240,234,216,0.06)', paddingTop: '72px', paddingBottom: '40px' }}>
      <div style={{ maxWidth: '1360px', margin: '0 auto', padding: '0 clamp(24px,5vw,80px)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '48px 40px', marginBottom: '64px' }}>
          ...
          {/* Brand */}
          <div style={{ ...col }}>
            <img src={window.__resources&&window.__resources.oliveLogo||'src/assets/olive-logo.png'} alt="Olive Studios" className="footer-logo" style={{ height: '100px', width: 'auto', maxWidth: '220px', objectFit: 'contain', cursor: 'pointer', marginBottom: '8px' }} onClick={() => navigate('home')} />
          ...
          <div style={{ ...col }}>
            ...
            {submitted ? (
              ...
              <form onSubmit={e => { e.preventDefault(); if (!email) return; try { window.OliveCollector && window.OliveCollector.submit('newsletter', { email: email, source: 'footer' }); } catch (err) {} setSubmitted(true); }} style={{ display: 'flex', gap: 0, marginTop: '4px' }}>
