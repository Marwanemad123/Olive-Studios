      ...
      <div style={{ minHeight: '75vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 'clamp(80px,10vw,140px) clamp(20px,5vw,72px) clamp(48px,6vw,80px)' }}>
        <FadeUp>
          ...
          {joined ? (
            ...
            <form onSubmit={e => { e.preventDefault(); if (!email) return; try { window.OliveCollector && window.OliveCollector.submit('collectors_circle', { name: name, email: email }); } catch (err) {} setJoined(true); }} style={{ maxWidth: '380px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
