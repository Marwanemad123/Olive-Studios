        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(40px,6vw,80px)' }}>
          ...
          <FadeUp delay={0.15}>
            ...
            {sent ? (
              ...
              <form onSubmit={e => { e.preventDefault(); try { window.OliveCollector && window.OliveCollector.submit('contact', { firstName: form.firstName, lastName: form.lastName, email: form.email, subject: form.subject, message: form.message }); } catch (err) {} setSent(true); }} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
